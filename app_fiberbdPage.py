...
class fiberbdPage(GUI):  # 繼承GUI
    def __init__(self, parent, controller):
        GUI.__init__(self, parent)
        self.name = 'dataPage'

        frame1 = tk.LabelFrame(self, frame_styles, text="工卡胚布資料預覽")
        ...
        def Load_data(self):
            Refresh_data()
            controller.dataF, controller.headF,  = dB.FiberBuild(self.path)
            #建立胚布重量字典
            controller.E2mass = dB.app_fiberDict(controller.dataF)
            column_list_account = list(controller.headF)
            tv1['columns'] = column_list_account 
            for column in column_list_account:
                tv1.heading(column, text=column)
                tv1.column(column, width=140)
            for row in controller.dataF[:50]:
